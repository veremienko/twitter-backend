...
- [x] **Фінал: підписки + fan-out стрічки** — таблиця follows з лічильниками,
      fan-out on write vs on read — збирає разом усі попередні теми.
      `follows` (followerId, followeeId, unique-констрейнт — той самий
      патерн, що `likes`) живе в user-service — природний власник зв'язків
      між користувачами; `followerCount`/`followingCount` на `users`
      оновлюються в тій самій транзакції, що insert/delete у `follows`,
      той самий insert+лічильник патерн, що `postLike`. Фан-аут — у
      twit-service (власник твітів і стрічки): `home_feed` (followerId,
      twitId) заповнюється в `createTwit` по одному рядку на поточного
      фолловера автора — **fan-out on write**, вартість пропорційна
      кількості фолловерів, оплачується одразу при твіті; `/feed/home`
      потім читає її простим індексованим запитом. `/feed/on-read` —
      протилежність: жодної таблиці, `follows` резолвиться в
      user-service на кожен запит, і `twits` фільтрується
      `WHERE author_id IN (...)` — дешевий запис, дорожче й крихкіше
      читання. Грабля, підтверджена вручну: список фолловерів
      береться до відкриття транзакції (мережевий виклик усередині
      транзакції тримав би конекшн і лок), тому падіння user-service під
      час `createTwit` губить лише фан-аут-рядок для цього твіта — сам
      твіт все одно створюється, і `/feed/on-read` (що не залежить від
      `home_feed`) все одно його бачить, а `/feed/home` — ні, доки твіт не
      перестворять. Це і є реальна демонстрація компромісу: on-write платить
      наперед і переживає збій читання, on-read нічого не платить наперед,
      але кожне читання залежить від user-service, і `/feed/on-read`
      відповідає 503, якщо `following/ids` недоступний, а `/feed/home` —
      ні (він не звертається до user-service за самою стрічкою взагалі,
      тільки за іменами авторів, і це деградує, а не падає).
